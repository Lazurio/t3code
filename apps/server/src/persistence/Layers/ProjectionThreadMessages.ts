...
import { ChatAttachment, ContextFile } from "@t3tools/contracts";
...
const ProjectionThreadMessageDbRowSchema = ProjectionThreadMessage.mapFields(
  Struct.assign({
    ...
    contextFiles: Schema.NullOr(Schema.fromJsonString(Schema.Array(ContextFile))),
  }),
);
...
function toProjectionThreadMessage(
  ...
  return {
    ...
    ...(row.contextFiles !== null ? { contextFiles: row.contextFiles } : {}),
  };
}
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextContextFilesJson =
        row.contextFiles !== undefined ? JSON.stringify(row.contextFiles) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextContextFilesJson},
            (
              SELECT context_files_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          context_files_json = COALESCE(
            excluded.context_files_json,
            projection_thread_messages.context_files_json
          ),
    ...
    execute: ({ messageId }) =>
      sql`
        SELECT
          ...
          context_files_json AS "contextFiles",
